...
- [Feature] **Git hooks security protection** - Automatically mounts `.git/hooks` as read-only by default to prevent containers from modifying git hooks that could execute malicious code on the host during git operations (commits, pushes, etc.). This protects against a class of supply chain attacks where a compromised AI coding tool could inject malicious hooks. Use `--writable-git-hooks` flag to opt-out when needed (e.g., if you need the AI to help configure git hooks). Can also be disabled via config: `[git] protect_hooks = false`. Includes comprehensive integration tests (19 tests) covering attack vectors like symlinks, hardlinks, and file operations. (#128)
